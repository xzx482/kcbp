...
class 主(QThread):
	...
	def run(s):
		...
		while True:
			...
			#'''
			t=time.time()
			if t-s.配置l['wd_']['时间']>7200 and (18*60+27)*60+40<获取秒(time.localtime(t))<(18*60+34)*60 and 课程表状态['上课'] and 0<课程表状态['上课']<=2:#时间在18:28-18:34之间 且 预备或者已上课
				s.配置l['wd_']['时间']=t
				当前课程=课程表状态['当前课程']
				#当前课程='外语听力'
				if 当前课程=='新闻周刊':
