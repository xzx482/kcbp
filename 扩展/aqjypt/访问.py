...
def 访问(url,请求头={},请求方法=None,数据=None,cookie=None,自动解码=True):
	'''
...
'''
	...
	while 1:
		...
		#'''
			...
			if 出错停止次数==-1 or 出错<出错停止次数:
				...
				print("网络错误:"+str(错误)+", 连续第 "+str(出错)+" 次出错, 暂停 "+str(出错暂停时间*10)+" 秒")
				time.sleep(出错暂停时间*10)
	#"""
	#响应=request.urlopen(请求)
	...
	return [响应.geturl(),响应.getcode(),响应头_原始,响应头_整理,响应头_cookie_字符串,响应头_cookie_整理,解码(响应.read())if 自动解码 else 响应.read()]
